...
def test_create_lookup_tables(create_lookup_tables):
    test_text = '''
        ...
        Moe_Szyslak Listen you little puke One of these days I'm gonna catch you and I'm gonna carve my name on your
...
def test_rnn(RNN, train_on_gpu):
    ...
    assert_message = 'Wrong hidden state size. Expected type {}. Got type {}'.format(correct_hidden_size,
                                                                                     hidden[0].size())
...
def test_forward_back_prop(RNN, forward_back_prop, train_on_gpu):
    ...
    assert (hidden_out[0][0] == hidden[0][
        0]).sum() == batch_size * hidden_dim, 'Returned hidden state is the incorrect size.'
