...
int main() {
  ...
  for (auto img_index = 0; img_index <= img_end_index - img_start_index;
       img_index++) {
    // TODO: Create ImageGenerator class responsible of providing an image at a
    // time.
    ...
    img_full_filename << img_base_path << img_prefix << img_number.str()
                      << img_file_type;
    ...
    if (detector_type == "SHITOMASI") {
      ...
      DetectKeypointsModern(keypoints, img_gray, detector_type);
    }
